...
5. Click `Run SAM3` for whole-image prompt inference on the selected image.
6. Review the draft annotations and correct boxes/classes in the canvas or
   Annotation tab.
7. If a box/class is edited, the old segmentation becomes stale and is excluded
   from segmentation export.
8. For poor or stale masks, select the annotation and click
   `Re-segment from Box` to generate a new mask/polygon from the current bbox.
...
without manual polygon editing. `Run SAM3` creates whole-image draft
annotations from the class prompts. Editing a bbox or class makes the existing
segmentation stale, hides it from the canvas, and excludes it from segmentation
export. `Re-segment from Box` is selected-box mask/polygon refinement: it keeps
the current bbox/class and replaces `polygon_xyn` with a new SAM3 polygon.
`Reset to SAM3` restores the original SAM3 annotation snapshot, including bbox,
class, and polygon.
...
|-- segmentation_skipped_report.json
`-- run_summary.json
```
...
- `yolo_labels/segmentation/*.txt`: YOLO segmentation labels for annotations
  with valid `polygon_xyn` segmentation. Edited stale polygons and bbox-only
  annotations are excluded from segmentation export. Edited, manual, and
  imported annotations are bbox-only unless they are re-segmented from the
  current box.
- `segmentation_skipped_report.json`: written when segmentation export skips
  stale, missing, or invalid polygons. It lists image, annotation id, class, and
  skip reason.
...
Detection export always uses the current editable `box_xyxy` state.
Segmentation export only includes annotations marked `segmentation_valid=True`
with valid `polygon_xyn`, including original SAM3 polygons and masks
regenerated with `Re-segment from Box`. Stale, missing, or invalid
segmentations are skipped and reported instead of being silently exported.
